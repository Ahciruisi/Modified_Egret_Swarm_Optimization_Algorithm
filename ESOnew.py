...
class ESO:
    ...
    def callFunc(self, x):
        ...
        if (len(y.shape) == 0):
            ...
            # print("new y.shape", np.tile(y, self.population_size).T.shape)
        return y
    ...
    def updateSurface(self):
        # print("in update surface")
        self.y = self.callFunc(self.x)
        # print("y dimensions after calling function: ", self.y.shape)
        self.y_hat = np.sum(self.w*self.x, axis=1)
        # print("y_p dimensions: ", self.y_hat.shape)
        self.error_history.append(np.abs(self.y-self.y_hat).min())
        # print("error history dimensions: ", len(self.error_history))
        p = self.y_hat-self.y
        # print("delta dimensions: ", p.shape)
        p = self.refill(p)
        # print("p dimensions after refill: ", p.shape)
        g_temp = p*self.x
    ...
    def randomSearch(self):
        ...
        # x_n = self.x + np.tan(r) * self.hop * decay * 0.5
    ...
    def adviceSearch(self):
        x_o = self.x + np.exp(-self.current_iter/(0.1*self.max_iter)) * 0.1 * self.hop * self.g
